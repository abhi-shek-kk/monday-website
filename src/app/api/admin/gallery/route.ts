...
import { getAdminGalleryItems } from "@/lib/services/admin.service";
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ success: true, galleryItems: items });
  } catch (err: unknown) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const statusInput = (formData.get("status") as string | null) || "PUBLISHED";
      ...
      const status = statusInput === "DRAFT" ? PublicationStatus.DRAFT : PublicationStatus.PUBLISHED;
