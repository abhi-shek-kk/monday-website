  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative max-w-2xl mx-auto">
        <Search className="w-6 h-6 text-[#756860] absolute left-4 top-1/2 -translate-y-1/2" />
        <input
          type="text"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Search courses, faculty, students, projects, events, notes..."
          aria-label="Search courses, faculty, students, projects, events, and study notes"
          className="w-full pl-13 pr-12 py-4 rounded-2xl bg-white border border-[#EFEAE3] shadow-sm text-base text-[#1C1917] focus:outline-none focus:ring-2 focus:ring-[#1C1917]"
        />
        {loading && (
          <Loader2 className="w-5 h-5 text-[#756860] animate-spin absolute right-4 top-1/2 -translate-y-1/2" />
        )}
      </div>

      {/* CATEGORY FILTER TABS */}
      {query.trim() !== "" && (
        <div className="flex flex-wrap items-center justify-center gap-2 border-b border-[#EFEAE3] pb-4">
          {[
      ...
      <div className="space-y-8">
        {query.trim() === "" ? (
          <div className="p-12 rounded-3xl bg-white border border-[#EFEAE3] text-center space-y-3 max-w-xl mx-auto">
          ...
          <div className="space-y-8">
            ...
            {(activeFilter === "ALL" || activeFilter === "COURSES") && results.courses.length > 0 && (
              <div className="space-y-4">
                <h3 className="text-sm font-bold uppercase tracking-wider text-[#756860] flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {results.courses.map((item) => (
                    <Link
                      ...
                      className="p-5 rounded-2xl bg-white border border-[#EFEAE3] hover:border-[#1C1917] transition-all space-y-1 block shadow-xs"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {results.faculty.map((item) => (
                    <Link
                      ...
                      className="p-5 rounded-2xl bg-white border border-[#EFEAE3] hover:border-[#1C1917] transition-all space-y-1 block shadow-xs"
                    >
                      <h4 className="text-base font-bold text-[#1C1917]">{item.fullName}</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {results.projects.map((item) => (
                    <Link
                      ...
                      className="p-5 rounded-2xl bg-white border border-[#EFEAE3] hover:border-[#1C1917] transition-all space-y-1 block shadow-xs"
                    >
                      <h4 className="text-base font-bold text-[#1C1917]">{item.title}</h4>
